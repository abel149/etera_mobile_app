...
class QuickAddGaragesShopsSeeder extends Seeder
{
    /**
    ...
    {
        $this->command->info('Quick adding 5 garages and 5 shops...');
        ...
        $this->command->info('Quick garages and shops addition completed!');
    }
    ...
    private function createGarages()
    {
        ...
        foreach ($garages as $garage) {
            // Check if user with this email or store_id already exists
            if (!User::where('email', $garage['email'])->exists() && 
                ...
                $this->command->info("Created garage: {$garage['name']} - " . ($garage['approved'] ? 'APPROVED' : 'NOT APPROVED'));
            } else {
                $this->command->info("Garage already exists: {$garage['name']} (email or store_id duplicate)");
            }
        }
    }
    ...
    private function createShops()
    {
        ...
        foreach ($shops as $shop) {
            ...
            if (!User::where('email', $shop['email'])->exists() && 
                ...
                $this->command->info("Created shop: {$shop['name']} - " . ($shop['approved'] ? 'APPROVED' : 'NOT APPROVED'));
            } else {
                $this->command->info("Shop already exists: {$shop['name']} (email or store_id duplicate)");
            }
        }
    }
}
