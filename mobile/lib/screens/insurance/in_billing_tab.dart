...
class InsuranceBillingTab extends StatefulWidget {
  const InsuranceBillingTab({super.key});

  @override
  State<InsuranceBillingTab> createState() => _InsuranceBillingTabState();
}

class _InsuranceBillingTabState extends State<InsuranceBillingTab> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    final res = await InsuranceService.getBilling();
    if (!mounted) return;
  ...
  Future<void> _changePlan(String newPlan) async {
    ...
    final res = await InsuranceService.updateBillingPlan(newPlan);
    if (!mounted) return;
...
class _StatementDetailScreenState extends State<_StatementDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final res =
        await InsuranceService.getStatementDetail(widget.sku);
    if (!mounted) return;
    if (res['success'] == true) {
      setState(() {
        _loading = false;
