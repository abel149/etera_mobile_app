...
class GarageBillingScreen extends StatefulWidget {
  const GarageBillingScreen({super.key});

  @override
  State<GarageBillingScreen> createState() => _GarageBillingScreenState();
}

class _GarageBillingScreenState extends State<GarageBillingScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    final res = await GarageService.getBillingOverview();
    if (!mounted) return;
    ...
    if (res['success'] == true) {
      setState(() {
        ...
        _currentPeriod = res['current_period'] is Map
            ...
            : null;
        final raw = res['statements'];
        _statements = raw is List
            ? raw.map((s) => Map<String, dynamic>.from(s as Map)).toList()
            : [];
        final rawInv = res['recent_invoices'];
        _recentInvoices = rawInv is List
            ? rawInv.map((i) => Map<String, dynamic>.from(i as Map)).toList()
            : [];
      });
    } else {
      setState(() {
  ...
  Future<void> _changePlan(String newPlan) async {
    ...
    final res = await GarageService.updateBillingPlan(newPlan);
    if (!mounted) return;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              _PlanOption(
                label: 'Per Invoice', value: 'per_invoice',
                ...
                : ListView(
                    ...
                    children: [
                      ...
                      if (_statements.isEmpty)
                        ...
                        ..._statements.map((s) => _StatementCard(
                              statement: s,
                              onTap: () => Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => _StatementDetailScreen(
                                      sku: s['sku'] as String),
                                ),
                              ),
                            )),
                      ...
                      if (_recentInvoices.isEmpty)
                        ...
                        ..._recentInvoices.map((inv) => _ProformaInvoiceCard(invoice: inv)),
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(label,
                    style: TextStyle(
                        fontWeight: FontWeight.w700,
                        color: selected ? EteraTheme.green : EteraTheme.textPrimary)),
                const SizedBox(height: 2),
                Text(description,
                    style: const TextStyle(fontSize: 12, color: EteraTheme.textMuted)),
              ]),
            ),
            if (selected) const Icon(Icons.check_circle, color: EteraTheme.green),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return EteraCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: Text(
            start.isNotEmpty && end.isNotEmpty ? '$start → $end' : 'Current billing period',
            style: const TextStyle(fontSize: 12, color: EteraTheme.textMuted),
          )),
        ...
        Row(children: [
          Expanded(child: _Stat('Proformas', '$count')),
          Expanded(child: _Stat('Subtotal', '${subtotal.toStringAsFixed(2)} Br')),
        ]),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _Stat('VAT (15%)', '${vat.toStringAsFixed(2)} Br')),
          Expanded(child: _Stat('Total Due', '${total.toStringAsFixed(2)} Br', highlight: true)),
        ]),
      ]),
    );
  }
...
class _StatementCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final total = (statement['total_amount'] ?? 0).toDouble();
        ...
        child: Padding(
          ...
          child: Row(children: [
            Container(
              ...
              child: const Icon(Icons.receipt_long, color: EteraTheme.green, size: 20),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                start.isNotEmpty && end.isNotEmpty ? '$start → $end' : sku,
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('${total.toStringAsFixed(2)} Br',
                  style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
              const SizedBox(height: 4),
              _StatusBadge(status: status),
            ]),
            const SizedBox(width: 8),
            const Icon(Icons.arrow_forward_ios, size: 14, color: EteraTheme.textMuted),
          ]),
        ),
      ),
    );
  ...
  static String _periodLabel(String type) {
    switch (type) {
      case 'monthly': return 'Monthly';
      case 'weekly': return 'Weekly';
      default: return 'Per Invoice';
...
class _StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (status.toLowerCase()) {
      case 'paid':
        bg = EteraTheme.green.withValues(alpha: 0.12); text = EteraTheme.green; break;
      case 'overdue':
        bg = EteraTheme.error.withValues(alpha: 0.1); text = EteraTheme.error; break;
      default:
        bg = EteraTheme.teal.withValues(alpha: 0.12); text = EteraTheme.teal;
    }
    return Container(
...
class _StatementDetailScreenState extends State<_StatementDetailScreen> {
  ...
  Future<void> _load() async {
    setState(() { _loading = true; _error = null; });
    final res = await GarageService.getStatementDetail(widget.sku);
    if (!mounted) return;
    if (res['success'] == true) {
      setState(() {
        _loading = false;
        ...
        _invoices = raw is List
            ? raw.map((i) => Map<String, dynamic>.from(i as Map)).toList()
            : [];
      });
    } else {
      setState(() {
      ...
      body: _loading
          ...
          : _error != null
              ? Center(
                  child: Text(_error!,
                      style:
                          const TextStyle(color: EteraTheme.textMuted)))
              : RefreshIndicator(
                  color: EteraTheme.green,
                  onRefresh: _load,
                  child: ListView(
                    ...
                    children: [
                      ...
                      Text(
                          'Proforma Invoices (${_invoices.length})',
                          style:
                              Theme.of(context).textTheme.titleLarge),
                      const SizedBox(height: 12),
    ...
    return EteraCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Expanded(child: _Mini('VAT', '${vat.toStringAsFixed(2)} Br')),
...
// ─── Proforma invoice card (per-invoice / overview invoices section) ──────────
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                brand.isNotEmpty
                    ...
                    : (file.isNotEmpty ? 'File #$file' : invoice['sku']?.toString() ?? ''),
                style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13),
              ),
              if (file.isNotEmpty)
                Text('File #$file',
