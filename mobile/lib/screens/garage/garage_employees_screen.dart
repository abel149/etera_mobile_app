...
class GarageEmployeesScreen extends StatefulWidget {
  const GarageEmployeesScreen({super.key});

  @override
  State<GarageEmployeesScreen> createState() => _GarageEmployeesScreenState();
}

class _GarageEmployeesScreenState extends State<GarageEmployeesScreen> {
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final result = await GarageService.listEmployees();
    if (!mounted) return;
  ...
  Future<void> _confirmDelete(User employee) async {
    ...
    final res = await GarageService.deleteEmployee(employee.id);
    if (!mounted) return;
  ...
  void _openAddSheet() {
    showModalBottomSheet(
      ...
      builder: (_) => SafeArea(
        top: false,
        child: _AddEmployeeSheet(onAdded: () {
          ...
          Center(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const Text(
                ...
                style:
                    TextStyle(color: EteraTheme.textMuted, fontSize: 13),
...
class _AddEmployeeSheetState extends State<_AddEmployeeSheet> {
  ...
  Future<void> _submit() async {
    ...
    final res = await GarageService.createEmployee(
    ...
    if (res['success'] == true) {
      ...
      String msg = res['message']?.toString() ?? 'Failed';
      if (errors is Map) {
