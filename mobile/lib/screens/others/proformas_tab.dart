...
class _OthersProformasTabState extends State<OthersProformasTab>
  ...
  Future<void> _load() async {
    ...
    final result = await OthersService.getProformas();
    if (!mounted) return;
  ...
  Widget _buildBody() {
          ...
          Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text('No proformas yet', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                const SizedBox(height: 8),
                const Text('Pull down to refresh or create a new request',
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) => _ProformaCard(
        item: _items[i],
        onTap: () async {
          await Navigator.pushNamed(context, '/proforma-detail', arguments: _items[i].id);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  const Icon(Icons.confirmation_number_outlined, size: 14, color: EteraTheme.textMuted),
                  const SizedBox(width: 4),
                  Text(item.licensePlate, style: const TextStyle(fontSize: 13, color: EteraTheme.textMuted)),
                  const SizedBox(width: 16),
                  const Icon(Icons.build_outlined, size: 14, color: EteraTheme.textMuted),
                  const SizedBox(width: 4),
                  Text('${item.parts.length} part(s)', style: const TextStyle(fontSize: 13, color: EteraTheme.textMuted)),
                ],
              ),
              const SizedBox(height: 6),
              Row(
                children: [
                  const Icon(Icons.directions_car_outlined, size: 14, color: EteraTheme.textMuted),
                  const SizedBox(width: 4),
                  Text(item.carType, style: const TextStyle(fontSize: 13, color: EteraTheme.textMuted)),
                  const Spacer(),
                  Text(item.shortDate, style: const TextStyle(fontSize: 12, color: EteraTheme.textMuted)),
                ],
              ),
            ],
          ),
