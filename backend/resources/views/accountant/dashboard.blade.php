    ...
    console.log(
        ...
        JSON.stringify(@json($invoices->toArray()), null, 2)
                    ...
                    <div class="modal-body">
                        ...
                        {{-- MARK AS PAID --}}
                        <form method="GET" action="{{ route('finance.markPaid', $summary->user->id) }}" class="mb-2">
                            <button class="btn btn-success btn-sm" {{ $summary->remaining <= 0 ? 'disabled' : '' }}>
                              Mark as Paid
                            </button>
                        </form>
                        ...
                        @if($summary->role === 'insurance')
                            <form method="POST" action="{{ route('finance.receivePayment', $summary->user->id) }}" class="mb-3">
                                @csrf
                                <button type="submit" class="btn btn-primary btn-sm" {{ $summary->insurance_proforma_unpaid <= 0 ? 'disabled' : '' }}>
                                    Payment Received from insutrance
                                </button>
                            </form>
                        @endif
